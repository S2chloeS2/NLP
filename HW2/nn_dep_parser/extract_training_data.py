...
pos_tags = ['<NULL>', '<ROOT>', 'CC', 'CD', 'DT', 'EX', 'FW', 'IN', 'JJ', 'JJR', 'JJS',
...
class FeatureExtractor(object):
    ...
    def __init__(self, word_vocab_file):
        ...
        self.pos_vocab = {tag: i for i, tag in enumerate(pos_tags)}
    ...
    def get_input_representation(self, words, pos, state):
        ...
        null_pos_index = self.word_vocab_size + self.pos_vocab['<NULL>']

        def word_to_index(word_id):
            if word_id == 0:
                return self.word_vocab['<ROOT>']
            p = pos[word_id]
            w = words[word_id].lower()
            if p == 'CD':
                return self.word_vocab['<CD>']
            elif p == 'NNP':
                return self.word_vocab['<NNP>']
            elif w in self.word_vocab:
                return self.word_vocab[w]
            else:
                return self.word_vocab['<UNK>']

        def pos_to_index(word_id):
            if word_id == 0:
                return self.word_vocab_size + self.pos_vocab['<ROOT>']
            p = pos[word_id]
            return self.word_vocab_size + self.pos_vocab.get(p, 0)

        result = []
        # Top 3 of stack: word indices
        for i in [1, 2, 3]:
            if len(state.stack) >= i:
                result.append(word_to_index(state.stack[-i]))
            else:
                result.append(null_index)
        # Top 3 of buffer: word indices
        for i in [1, 2, 3]:
            if len(state.buffer) >= i:
                result.append(word_to_index(state.buffer[-i]))
            else:
                result.append(null_index)
        # Top 3 of stack: POS indices
        for i in [1, 2, 3]:
            if len(state.stack) >= i:
                result.append(pos_to_index(state.stack[-i]))
            else:
                result.append(null_pos_index)
        # Top 3 of buffer: POS indices
        for i in [1, 2, 3]:
            if len(state.buffer) >= i:
                result.append(pos_to_index(state.buffer[-i]))
            else:
                result.append(null_pos_index)
